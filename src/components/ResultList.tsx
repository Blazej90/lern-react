...
const ResultsList: React.FC<{
  results: { text: string; time: number }[];
  interimResult: string | null;
  setIsLoading: (loading: boolean) => void; // Nowy prop do śledzenia stanu ładowania
}> = ({ results, interimResult, setIsLoading }) => {
  // Filtrujemy unikalne wyniki
  const uniqueResults = results.filter(
    (result, index, self) =>
      index ===
      self.findIndex((r) => r.text === result.text && r.time === result.time)
  ...
  // Jeśli `interimResult` istnieje, ustawiamy `isLoading` na true
  React.useEffect(() => {
    setIsLoading(!!interimResult); // Ustawienie stanu ładowania na podstawie interimResult
